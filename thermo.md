...
Statistical thermodynamics explores the way in which thermal properties of a system emerge from the physics of its elementary objects. If the system is an air-filled balloon, for example, it's thermal properties are things like temperature, pressure and volume, while its elementary objects are atoms and molecules.

Thermal properties are relatively intuitive—we can feel the difference between hot and cold, we can see steam rising from a boiling pot of water, and we can watch a helium-filled balloon pull whatever it's attached to. But that these things should occur due to the motion of atoms is far from obvious, and if we tried to predict any of these phenomena using classical or quantum mechanics directly, we wouldn't get very far—there are too many interacting objects to consider.
...
Also, the word _thermodynamics_ should be defined. Thermodynamics is about thermal things changing, like when energy is absorbed or given up, or when volume is increased or decreased, etc. But changes are assumed to happen in a special way. They happen such that the system remains effectively in equilibrium at all steps throughout its dynamics. "Effectively" in the sense that we can treat the system thermally, and not have to worry about transient dynamics or This is acheived by changing the system _very_ slowly, so any transients dissipate . hen the system is effectively in equilibrium throughout its trajectory, the process is called "quasi-static". In practice, it's accomplished by changing the system _very_ slowly. In fact, to acheive  

the system effectively stays in equilibrium throughout its trajectory. in a way that ensures the system be in equilibrium at all times. allows the system to equilibrate at every (infinitesimal) step along its trajectory. The reason for this is because we don't want any transient dynamics to influence the system's behavior. Such a process is called "quasi-static", and it's satisfied by changing the system very slowly. Strictly speaking, it takes infinitely long to do something quasi-statically, but in practice we can get close enough on practical time scales.  (Side note—although it's tempting to synonymize "dynamic equilibrium" with quasistatic due to the system being in equilibrium throughout its dynamics, really dynamic equilibrium means something different.)

Now, in order to start _doing_ thermodynamics, we need its fundamental postulate: 

> In thermal equilibrium, each microstate is equally likely to be observed at all times.
