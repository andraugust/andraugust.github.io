...
The principles of QM are formulated around the idea of measurables, i.e., the outcomes of experiments. They state that:

* Measurables (such as spin) are represented by Hermitian operators, which are defined as having the property $$\mathbf{H^\dagger} = \mathbf{H}$$.
* Quantum states (such as $$\ket{u}$$ and $$\ket{d}$$) are eigenvectors of these operators.
* The measurable quantities themselves (such as $$\pm1$$) are the eigenvalues.
